...
const martData = {
    happy: {
        ...
        rating: 4.6,
        reviewCount: 128,
        productIds: ['egg', 'milk', 'prod_fruit_2', 'prod_fruit_4', 'prod_veg_2', 'prod_veg_3'],
        reviews: [
            { name: '동네주민A', stars: 5, text: '채소가 항상 신선하고 저렴해서 자주 이용합니다. 주차장도 넓어서 좋아요.' },
            { name: '알뜰장보기', stars: 4, text: '계란 특가가 자주 올라와서 장보기 메모에 꼭 넣어둡니다. 계산대 회전도 빠른 편이에요.' }
        ]
    },
    hanaro: {
        name: '하나로마트 율량점',
        ...
        rating: 4.4,
        reviewCount: 96,
        productIds: ['strawberry', 'prod_fruit_3', 'prod_veg_1', 'prod_processed_4'],
        reviews: [
            { name: '율량맘', stars: 5, text: '과일 상태가 균일하고 포장도 깔끔해요. 딸기 행사 때마다 만족도가 높습니다.' },
            { name: '장바구니요정', stars: 4, text: '농산물 코너가 넓어서 비교하며 고르기 좋고, 주말 행사 품목이 괜찮아요.' }
        ]
    },
    daeryuk: {
        name: '대륙식자재마트',
        img: 'https://loremflickr.com/800/500/supermarket,store?lock=10',
        rating: 4.3,
        reviewCount: 72,
        productIds: ['new_prod_0_0', 'new_prod_0_1'],
        reviews: [
            { name: '식당사장님', stars: 5, text: '대용량 식용유랑 양파 가격이 좋아서 업장 장보기로 자주 갑니다.' },
            { name: '소분모임장', stars: 4, text: '박스 단위 상품이 많아 이웃들과 나눠 사기 좋아요. 카트 공간도 넉넉합니다.' }
        ]
    },
    angel: {
        name: '천사 식자재마트',
        img: 'https://loremflickr.com/800/500/supermarket,store?lock=20',
        rating: 4.5,
        reviewCount: 84,
        productIds: ['new_prod_1_0', 'new_prod_1_1'],
        reviews: [
            { name: '원주장보기', stars: 5, text: '행사 가격이 눈에 잘 띄고 직원분들이 재고 위치를 친절히 알려주세요.' },
            { name: '절약러', stars: 4, text: '양파망이 저렴해서 김치 담글 때 유용했습니다. 신선도도 괜찮았어요.' }
        ]
    },
    sangji: {
        name: '상지 식자재마트',
        img: 'https://loremflickr.com/800/500/supermarket,store?lock=30',
        rating: 4.2,
        reviewCount: 58,
        productIds: ['new_prod_2_0', 'new_prod_2_1'],
        reviews: [
            { name: '상지동주민', stars: 4, text: '대용량 상품 가격이 안정적이고 저녁 늦게 가도 기본 품목은 잘 남아 있어요.' },
            { name: '반찬가게준비중', stars: 5, text: '식자재 가격 비교할 때 제일 먼저 확인하는 곳입니다. 행사 품목 구성이 실속 있어요.' }
        ]
    },
    lmart: {
        name: '엘마트',
        img: 'https://loremflickr.com/800/500/supermarket,store?lock=40',
        rating: 4.1,
        reviewCount: 43,
        productIds: ['new_prod_3_0', 'new_prod_3_1'],
        reviews: [
            { name: '엘마트단골', stars: 4, text: '생활용품이랑 식자재를 한 번에 사기 편합니다. 전단 행사도 자주 바뀌어요.' },
            { name: '주말장보기', stars: 4, text: '동선이 단순해서 빠르게 장보기 좋고, 대용량 양파 가격이 만족스러웠습니다.' }
        ]
    }
};

let currentMartKey = 'happy';
let currentProductId = null;
let pendingReviewTarget = '행복한 식자재마트';
...
window.openProductDetail = function(productId) {
    ...
    const martName = data.mart || (data.comparisons && data.comparisons[0] ? data.comparisons[0].mart : '추천 상품');
    const badgeHtml = `<span style="margin-right: 8px;"><i class="fas fa-store"></i> ${martName}</span> <span style="background:var(--primary); color:white; padding:2px 6px; border-radius:4px; font-size:0.8rem;">${data.badge}</span>`;
...
function getProductCleanTitle(product) {
    if (!product) return '';
    let cleanTitle = product.title || '';
    if (product.mart) {
        cleanTitle = cleanTitle.replace(new RegExp(product.mart, 'g'), '').trim();
    }
    return cleanTitle.replace(/^[\[\]\s]+/, '');
}
...
function getDefaultProductReviews(product) {
    const martName = product.mart || (product.comparisons && product.comparisons[0] ? product.comparisons[0].mart : '동네마트');
    const title = getProductCleanTitle(product);
    return [
        { name: '꼼꼼후기', stars: 5, text: `${title} 가격이 전단지와 같고 포장 상태도 깔끔했습니다. ${martName} 행사 상품 중 만족도가 높아요.` },
        { name: '오늘장보기', stars: 4, text: `할인율이 좋아서 바로 담았습니다. 신선도와 양 모두 데일리 장보기용으로 충분합니다.` }
    ];
}
    ...
    container.innerHTML = reviews.map(review => `
        <div class="review-item">
            <div class="review-header">
                ...
                <span class="stars text-yellow">${getStars(review.stars)}</span>
            </div>
...
function renderProductReviews(productId) {
    ...
    if (!product.reviews) {
        product.reviews = getDefaultProductReviews(product);
    }
    
    const reviewCount = product.reviews.length;
    const average = product.reviews.reduce((sum, review) => sum + review.stars, 0) / reviewCount;
    const titleEl = document.getElementById('pd-reviews-title');
    const scoreEl = document.getElementById('pd-review-score');
    
    if (titleEl) titleEl.textContent = `구매자 리뷰 (${reviewCount}건)`;
    if (scoreEl) scoreEl.textContent = `${average.toFixed(1)} / 5.0`;
    renderReviewList('pd-review-list', product.reviews);
}

function getProductsForMart(mart) {
    const productsById = mart.productIds
        .map(id => productData[id])
        .filter(Boolean);
    const matchedProducts = Object.values(productData).filter(product => product.mart === mart.name);
    
    return [...productsById, ...matchedProducts]
        .filter((product, index, list) => list.findIndex(item => item.id === product.id || item.title === product.title) === index)
        .slice(0, 6);
}

function renderMartProducts(mart) {
    const container = document.getElementById('md-products');
    if (!container) return;
    
    const products = getProductsForMart(mart);
    if (products.length === 0) {
        container.innerHTML = '<div class="empty-state">현재 진행중인 핫딜이 없습니다.</div>';
        return;
    }
    
    container.innerHTML = products.map(product => {
        const cleanTitle = getProductCleanTitle(product);
        return `
            <div class="product-item" onclick="openProductDetail('${product.id}')" style="cursor:pointer;">
                <img src="${product.img}" alt="${cleanTitle}" class="prod-img">
                <div class="prod-info">
                    <div class="prod-badge">${product.discount || product.badge} OFF</div>
                    <h3>${cleanTitle}</h3>
                    <p class="prod-price"><strong>${product.price}</strong> <del>${product.originalPrice}</del></p>
                </div>
                <button class="add-cart-btn" onclick="event.stopPropagation(); window.addToCart('${cleanTitle}', '${mart.name}', '${product.price}')"><i class="fas fa-plus"></i></button>
            </div>
        `;
    }).join('');
}

window.openMartDetail = function(martKey) {
    const mart = martData[martKey] || martData.happy;
    currentMartKey = martKey || 'happy';
    pendingReviewTarget = mart.name;
    
    const coverImg = document.getElementById('md-cover-img');
    const nameEl = document.getElementById('md-name');
    const ratingEl = document.getElementById('md-rating');
    
    if (coverImg) coverImg.src = mart.img;
    if (nameEl) nameEl.textContent = mart.name;
    if (ratingEl) ratingEl.innerHTML = `<i class="fas fa-star text-yellow"></i> ${mart.rating.toFixed(1)} (리뷰 ${mart.reviewCount}개)`;
    
    renderMartProducts(mart);
    renderReviewList('md-reviews', mart.reviews);
    navigateTo('view-mart-detail');
};

window.openReviewForCurrentMart = function() {
    const mart = martData[currentMartKey] || martData.happy;
    pendingReviewTarget = mart.name;
    setReviewTarget(pendingReviewTarget);
    navigateTo('view-write-review');
};

window.openReviewForCurrentProduct = function() {
    const product = productData[currentProductId];
    pendingReviewTarget = product ? getProductCleanTitle(product) : '상품 리뷰';
    setReviewTarget(pendingReviewTarget);
    navigateTo('view-write-review');
};

function setReviewTarget(targetName) {
    const select = document.getElementById('review-target');
    if (!select) return;
    
    let option = [...select.options].find(item => item.value === targetName || item.textContent === targetName);
    if (!option) {
        option = new Option(targetName, targetName);
        select.add(option, 0);
    }
...
window.submitReview = function() {
    ...
    const selectedStars = document.querySelectorAll('.star-rating i.active').length || 5;
    const targetName = select ? select.value : pendingReviewTarget;
    const reviewText = textarea && textarea.value.trim()
        ? textarea.value.trim()
        : '가격이 좋고 상품 상태도 만족스러웠습니다.';
    const newReview = { name: '나의 리뷰', stars: selectedStars, text: reviewText };
    
    const martEntry = Object.values(martData).find(mart => mart.name === targetName);
    if (martEntry) {
        martEntry.reviews.unshift(newReview);
        martEntry.reviewCount += 1;
    } else if (currentProductId && productData[currentProductId]) {
        if (!productData[currentProductId].reviews) {
            productData[currentProductId].reviews = getDefaultProductReviews(productData[currentProductId]);
        }
        productData[currentProductId].reviews.unshift(newReview);
    }
    ...
    alert('리뷰가 등록되었습니다. 50P 지급 완료!');
    
    if (martEntry) {
        const key = Object.keys(martData).find(martKey => martData[martKey] === martEntry);
        window.openMartDetail(key);
    } else if (currentProductId) {
        window.openProductDetail(currentProductId);
    } else {
        navigateTo('view-mypage');
    }
};
